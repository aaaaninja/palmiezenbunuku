  ...
  for (const [chapter_name, chapter_url] of chapter_name_and_urls) {
    ...
    const [section, lesson] =
      JSON.parse(await page.$eval('div[data-react-class="App.Chapter"]', el => el.getAttribute('data-react-props')))
          ...
          .chapter
          .class_headers
    section.sections.filter(el => el.resource_type === 'guide').forEach(({ content_url }) => console.log(content_url)) // ここで出力されたものがvanilla-clipperによって処理される
    ...
    for (const { content_url, title } of section.sections.filter(el => el.resource_type === 'guide')) {
      ...
      await page.pdf( // https://github.com/puppeteer/puppeteer/issues/475
        { path: `${dir_name_content}/${title.replace(/[(\\|/|:|\\*|?|\"|<|>|\\\\|)]/g, '')}.pdf`
