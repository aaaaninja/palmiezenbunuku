...
////////////////////////////////////////////////////////////////////////////////
  ...
  const [result_video, result_slide, result_offer] = await Promise.all([
    spawnAsync('youtube-dl', ['-o', `${target_directory}/${c_number}_%(format)s_%(resolution)s.mp4`, '-f', 'bestvideo+audio-high-audio/bestvideo+audio-medium-audio/bestvideo+bestaudio', master_m3u8_url]),
  ...
  for (const chapter of video_urls) {
    ...
    const result_video = await spawnAsync('youtube-dl', ['-o', `${target_directory}/${c_number}_%(format)s_%(resolution)s.mp4`, '-f', 'bestvideo+audio-high-audio/bestvideo+audio-medium-audio/bestvideo+bestaudio', master_m3u8_url])
  }
